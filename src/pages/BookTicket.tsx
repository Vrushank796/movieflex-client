...
const BookTicket = () => {
  ...
  useEffect(() => {
    const fetchMovieTheatreData = async () => {
      try {
        const getMovieDataUrl = `${server_url}/movie/${id}`;
        const getTheatreListUrl = `${server_url}/theatreList`;
        const getMovieInTheatreListUrl = `${server_url}/movie-theatre-info/${movieBookingForm.location}`;
